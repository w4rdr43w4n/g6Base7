...
export async function searchInSemantic(query:string,maxResults=15,retries = 2, delay = 300):Promise<resData[]>{

// Searching for the query
try{
  let response;
  try {
 response = await axios.get(KEYS.SEMANTIC_CONFIG.api_url,  {params:{
  'query': `${query}`,
  'limit': maxResults,
},
...
const detResponse = await axios.post('https://api.semanticscholar.org/graph/v1/paper/batch?fields=title,year,authors,openAccessPdf,abstract', {
  ids: Ids,
  withCredentials:false,
  headers: {
    'x-api-key':KEYS.SEMANTIC_CONFIG.api_key
...
const rs:resData[] = []
  Metadata.forEach((paper:paper) => {
    ...
    const pdf_url = (paper.openAccessPdf === null)?'':paper.openAccessPdf.url
    rs.push({id:rs.length,title:title,author:author,authors:author_names,published:year,pdf_url:pdf_url,abstract:abstract})
    }
});
return rs
} catch (error) {
